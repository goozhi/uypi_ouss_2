        ...
        case /^sopc$/i.test(rj_nixb):
            ...
            if (/\byhrj\b/i.test(ebwu)) {
                var rj_yhld = diwr_vnwm_tsjq.map(rn1 => {
                    var rj_csrf_eowl_uxux="md-\n* -md eowl uxux: Bi tsjq ac eowl rjse."
                    if(rn1.eowl_uxux=="string"){
                        rj_csrf_eowl_uxux="md-\n* -md eowl uxux: Bi tsjq ja eowl wdbu ud dk rjse bqeo ae nvcm."
                    }
                    var rj_yhld_2 = "gg-"+rn1.zkrs+"\n=============="+"md-\n* -md cqpi fr: `" + rn1.vnwm_vr_afoa_lzm_wu.map(rn2 => {
                        return "md-" + rn2 + "-md"
                    }).join('` ae `') + "`:\n"
                    rj_yhld_2 += "md-\n* -md csrf: "+rn1.csrf + rj_csrf_eowl_uxux+"\nmd-* -md klch:\n-gg";
                ...
                var rj_yhld = diwr_vnwm_tsjq.map(rn1 => {
                    var rj_csrf_eowl_uxux="\n* eowl uxux: Bi tsjq ac eowl rjse."
                    if(rn1.eowl_uxux=="string"){
                        rj_csrf_eowl_uxux="\n* eowl uxux: Bi tsjq ja eowl wdbu ud dk rjse bqeo ae nvcm."
                    }
                    return ""+rn1.zkrs+"\n=============="+"\n* cqpi fr: `"+rn1.vnwm_vr_afoa_lzm_wu.map(rn2 => {
                        return "md-" + rn2 + "-md"
                    }).join('` ae `') + "`:\n" + "\n* csrf: "+rn1.csrf + rj_csrf_eowl_uxux+"\n* klch:\n" + '```js\n'+rn1.klch.replace(/\n[^\n\S]+/g, "\n").replace(/^\s+/,"")+'\n```\n'
                }).join("\n---------------\n")
                return rj_yhld
            }
