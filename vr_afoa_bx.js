...
const VR_TSJQ_ZV_TYUB_LDRG = require('./VR_TSJQ_ZV_TYUB_LDRG')
const VR_TSJQ_ZV_RJQT_GRBJ = require('./VR_TSJQ_ZV_RJQT_GRBJ')
const VR_TSJQ_ZV_RJQT_FDMJ = require('./VR_TSJQ_ZV_RJQT_FDMJ')
const VR_TSJQ_ZV_VKVY_LD_TYUB_VN_ZNZK_NQLL = require('./VR_TSJQ_ZV_VKVY_LD_TYUB_VN_ZNZK_NQLL')
const VR_TSJQ_ZV_VKVY_LD_TYUB_VN_ZNZK_NQUD = require('./VR_TSJQ_ZV_VKVY_LD_TYUB_VN_ZNZK_NQUD')
...
function vr_afoa_bx() {
    ...
    },
    {
        ...
        comment={{参数modify=translations表示修改译文}}
        60=modify=translations{{
            第一行请输入：中英对应库目录
            第二行请输入：翻译标准库目录
            第三行开始请输入: 需要修改的对象的编号，多个编号用空白符隔开
        }}
        comment={{参数modify=originals表示修改原文}}
        60=modify=originals{{
            第一行请输入：中英对应库目录
            第二行请输入：翻译标准库目录
            第三行开始请输入: 需要修改的对象的编号，多个编号用空白符隔开
    ...
    *exit*
    ...
    },
    {
        ...
        //参数为32或asc表示将会设置d4往后的值以30-39的规律循环赋值（但d4为2f），然后输出对应值（该效果仅对计算类型项目有效）。
        99=32{{
            *"变道盲区按键次数",   {22 b0 01},cf,d4*16777216+d5*65536+d6*256+d7,.0,次;
            *"车道偏离按键次数",   {22 b0 01},cf,d8*16777216+d9*65536+d10*256+d11,.0,次;
        }}
        ...
        //下面参数表示设置每条算式最高字节为开发者预设值，输出对应值（输出对应值仅对计算类型项目有效）。参数9可以用max代替。
        99=9{{
            *"变道盲区按键次数",   {22 b0 01},cf,d4*16777216+d5*65536+d6*256+d7,.0,次;
            *"车道偏离按键次数",   {22 b0 01},cf,d8*16777216+d9*65536+d10*256+d11,.0,次;
        }}
