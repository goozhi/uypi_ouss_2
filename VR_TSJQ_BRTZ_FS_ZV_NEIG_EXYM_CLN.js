...
function VR_TSJQ_BRTZ_FS_ZV_NEIG_EXYM_CLN(DIWR_AFOA) {
    ...
    var diwr_neig
    if (DIWR_AFOA.diwr_mcvn) {
        diwr_neig = DIWR_AFOA.diwr_mcvn.diwr_neig
    }
    var DS
    var BQEO_2 = BQEO_1.replace(/^\s*[`\u00b7]|[`\u00b7]\s*$/g, "")
    var RJSE_JTYJ
    if (VNWM_MCVN_1 == null) {
        RJSE_JTYJ = "config=mod{{`\n" + JSON.stringify(diwr_neig) + "\n`}}"
    } else {
...
module.exports = VR_TSJQ_BRTZ_FS_ZV_NEIG_EXYM_CLN;
