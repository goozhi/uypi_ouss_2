...
function RSCS_3f_NINI(NINI_IOWR_1) {
    if (typeof (NINI_IOWR_1) != 'object') {
        throw 'RSCS_3f_NINI : MCVN UXUX MSOX : ' + typeof (NINI_IOWR_1);
    }
    ...
    var NINI_3f_VNWM_MCVN = NINI_3f_VNWM_KP.map(RNSF => {
        ...
        if (IOWR_YHLD == null) {
            ...
            var jtyj_yg
            try {
                jtyj_yg = eval("0x" + IOWR_YHLD[1])
            } catch (err) {
                if(!/oth/i.test(IOWR_YHLD[1])){
                    throw new Error('csrf-jtyj yg ft pj-' + IOWR_YHLD[1])
                }else{
                    jtyj_yg=IOWR_YHLD[1]
                }
            }
            ...
            return { KLVQ_MCVN: IOWR_YHLD[1], BQEO: IOWR_YHLD[2], DIYC_KLVQ_ZV_YENH }
        }
    })
    var NINI_MCVN_IOWR_1 = { NINI_WU, NINI_ES, NINI_UXUX, NINI_UYPJ_1, NINI_3f_VNWM_KP };
    return { NINI_3f_VNWM_MCVN, NINI_MCVN_IOWR_1 };
