function NINI_CGNE(RJSE_MCNV_NINI, RJSE_NIXB_NINI) {
    ...
    VNWM_NIXB_NINI.forEach((RNSF, EQWY_2) => {
        ...
        for (var EQWY_1 = 0; EQWY_1 < IOWR_VNWM_MCNV_NINI.length; EQWY_1++) {
            var STST_MI = STST_MI_UYPJ(IOWR_VNWM_MCNV_NINI[EQWY_1].NINI_WU.replace(/\s/g,""), RNSF.replace(/\s/g,""));
            if (STST_MI < 0.55) {
            } else if (STST_MI == 1) {
    ...
    IOWR_VNWM_ZV_NINI_DIZC_NVCM.forEach((RNSF) => {
        ...
        if (VNWM_YBKC_2.indexOf(EQWY_YHLD_2) == -1 && VNWM_YBKC_1.indexOf(EQWY_YHLD_1) == -1) {
            ...
            VNWM_VDUM_NINI[RNSF.EQWY_ZV_VNWM_NIXB_NINI] =  RNSF.NIXB_NINI_WU + " 相似度 : " + RNSF.STST_MI.toFixed(3)+ " "+RNSF.IOWR_MCNV_NINI.MCNV_NINI_KP;
            return;
        }
    });
