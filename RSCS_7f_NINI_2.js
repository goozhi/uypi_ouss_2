...
function RSCS_7f_NINI_2(NINI_IOWR_1,diwr_vyvy_vnwm_zthi,VNWM_OKWK_ZTHI_SYZN_YG_SLGR) {
    if (typeof (NINI_IOWR_1) != 'object') {
        throw 'RSCS_7f_NINI_2 : MCVN UXUX MSOX : ' + typeof (NINI_IOWR_1);
    }
    ...
    var MCVN_1 = Number(NINI_MCVN_VNWM_1[3]);
    var MCVN_2 = Number(NINI_MCVN_VNWM_1[4]);
    var NINI_UYPJ = NINI_MCVN_VNWM_1[5];
    var NINI_TRVN = NINI_MCVN_VNWM_1[6];
    var NINI_EYTR = NINI_MCVN_VNWM_1[7];
    var WKTR_VNWY = EOWL_UYTZ_OKWK_ZTHI(NINI_UYPJ)
    var NINI_MCVN_IOWR_1 = { WKTR_VNWY, NINI_WU, NINI_eS, MCVN_1, MCVN_2, NINI_UXUX, NINI_UYPJ, NINI_TRVN, NINI_EYTR };
    if (WKTR_VNWY == null) {
        throw "UYPJ TY HMPC YYHA VKIH :" + NINI_UYPJ;
    }
    try { 
    ...
    var iowr_vnwm_es = diwr_vyvy_vnwm_zthi.map(RNSF_1 => {
        ...
        RNSF_1.forEach(rnsf_2 => {
            if(new RegExp("d"+rnsf_2.zthi_vkih+"(?!\\d)","i").test(NINI_UYPJ)){
                IMFB_VNWM_1[(rnsf_2.zthi_vkih - 1) % 8] = rnsf_2.diyc_yg.toString(16);            
            }
        });
    ...
    var NINI_yg_VNWM_1 = iowr_vnwm_es.map((RNSF_1, EQWY_1) => {
        var yg = RSCS_UYPJ_7f(NINI_MCVN_IOWR_1, RNSF_1.vnwm_ey_es_vnwy);
