...
function RSCS_10_NINI(NINI_IOWR_1, ZTHI_GTAW_VNWM) {
    if (typeof (NINI_IOWR_1) != 'object') {
        throw 'RSCS_10_NINI : MCVN UXUX MSOX : ' + typeof (NINI_IOWR_1);
    }
    ...
    var vnwm_NINI_10_mcvn_KP = NINI_MCVN_VNWM_1.slice(3, NINI_MCVN_VNWM_1.length)
    var NINI_10_MCVN = []
    vnwm_NINI_10_mcvn_KP.forEach(RNSF_1 => {
        ...
        if (IOWR_ZTHI_VKIH != null) {
            ...
            VNWM_ZTHI_VKIH.forEach(rnsf_2 => {
                ...
                NINI_10_MCVN.push(IOWR_ZTHI)
            })
        }else if(/^\s*d\d+\s*$/i.test(RNSF_1)){
            var vn_1=eval(RNSF_1.replace(/d/i,""))
            NINI_10_MCVN.push({ ZTHI_VKIH: vn_1, ZTHI_YG: ZTHI_GTAW_VNWM[vn_1], ZTHI_DIYC_YG: eval("0x"+ZTHI_GTAW_VNWM[vn_1]).toString(10) })
        }
        else {
            NINI_10_MCVN.push({ ZTHI_VKIH: null, ZTHI_YG: null, ZTHI_DIYC_YG: RNSF_1 })
        }
    })
    var NINI_MCVN_IOWR_1 = { NINI_WU, NINI_ES, NINI_UXUX, vnwm_NINI_10_mcvn_KP };
    return { NINI_10_MCVN, NINI_MCVN_IOWR_1 };

}
module.exports = RSCS_10_NINI