...
function SIIHGGUXGGRJSEGGBRTZGGFS_2(IOWR_1, VN_1) {
    ...
    let RJSE_1 = "";
    for (FO_1 in IOWR_1) {
        if (typeof (IOWR_1[FO_1]) == "string") {
            if (/^\s+$/.test(IOWR_1[FO_1])) {
            } else {
                var RJSE_YHLD = IOWR_1[FO_1].replace(/((?:\n|^))\s+/g, '$1').replace(/\r/g, '')//qingkongKonggeHeHuiche
                RJSE_1 += RJSE_YHLD.replace(/((?:\n(?!$)|^))/g, "$1" + Kongge.slice(0, VN_1));
            }
        } else if (typeof (IOWR_1[FO_1] == "object")) {
            try {
                RJSE_1 = RJSE_1 + "\{\n" + SIIHGGUXGGRJSEGGBRTZGGFS_2(IOWR_1[FO_1], VN_1 + 4);
                if (/\n$/.test(RJSE_1)) {
                ...
                throw err + "\n" + FO_1 + ":" + JSON.stringify(IOWR_1[FO_1]) + "\nWLQQ UMMS 回归出错";
            }
        } else {
