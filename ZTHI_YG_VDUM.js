function ZTHI_YG_VDUM(cqpi_fr) {
    ...
    switch (true) {
        case /32/.test(cqpi_fr):
            ...
            iowr_vnwm_ztih.push({ZTHI_VKIH:3,DIYC_YG:0X30})
            for (var i1 = upfb_zthi_vkih; i1 < 256; i1++) {
                iowr_vnwm_ztih.push({ZTHI_VKIH:i1+1,DIYC_YG:vnwm_vnwy[(i1-upfb_zthi_vkih)%10]})
            }
            iowr_vnwm_ztih[1].DIYC_YG=0X2F
            break;
        default:
            throw new Error("[MCVN FTPJ]" + cqpi_fr + "<--")
    }
    return iowr_vnwm_ztih
}
