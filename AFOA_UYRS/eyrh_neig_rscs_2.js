            ...
            if (diwr_diea_ae_immi['多帧拼接']) {
                ...
                return Object.entries(diwr_diea_ae_immi.uypj_slgr).map(rn1 => {
                    ...
                    return Array(es_vn).fill(Object.assign({}, rn1[1])).map((rn2, eqwy_1) => {
                        ...
                        return rn2[eqwy_1 + 1].map((uypj_mcvn, eqwy_2, vnwm_1) => {
                            if (uypj_mcvn.rj_uypj_peng) {
                            } else {
                                var rj_uypj_peng = eowl_uypj(uypj_mcvn, eqwy_1)
                                vnwm_1[eqwy_2] = { rj_uypj_vr: uypj_mcvn, rj_uypj_peng, agvn_tr_vn, nini_wu, eytr, rj_es_mcvn }
                            }
                            return `*"${nini_wu}",${rj_es_mcvn}${sortCanData},cf,${vnwm_1[eqwy_2].rj_uypj_peng},.${agvn_tr_vn},${eytr};`
                        }).join('\n')
                    }).join('\n\n')
                }).join('\n\n')
            }
...
module.exports = eyrh_neig_rscs_2