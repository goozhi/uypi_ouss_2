...
async function diwr_rjm_vyn_ld_peng(diwr_slm, diwr_neig_kp) {
    var vnwm_fo_1 = [
        'zkrs',
        'ljey_vkih'
    ]
    ...
    var rj_peng = diwr_slm.vnwm_vnwy.map(rn1 => {
        if(/\.peng$/i.test(rn1.zkrs)){
            ...
            uz_ms('csrf-rjm vyn ac huop bi ebwu-'+rn1.zkrs +'-kp-'+diwr_slm.yxna_yowr)
        }
    }).join('\n')
...
module.exports = diwr_rjm_vyn_ld_peng