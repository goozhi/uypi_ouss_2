...
async function diwr_vnwy_tu_ld_peng(diwr_slm, diwr_neig_kp) {
    ...
    var rj_vr = diwr_slm.vnwm_vnwy.map(rn1 => {
        if (diwr_neig_kp.diwr_wwcf[rn1.stats.mtimeMs]) {
            diwr_neig_kp.gkqj_acdb_zjzj = true;
        }
        return rn1.bqeo
    }).join('\n')
    var rj_peng
    ...
    if (!diwr_neig_kp.gkqj_acdb_zjzj) {
        var RJSE_MSOX = await ZJZJ_TSJQ_Str(rj_peng)
        if (/\S/i.test(RJSE_MSOX)) {
            uz_ms(diwr_slm.yxna_yowr + '\n' + RJSE_MSOX);
        } else {
            diwr_slm.vnwm_vnwy.map(rn1 => {
                diwr_neig_kp.diwr_wwcf[rn1.stats.mtimeMs] = true;
            })
        }
    }
