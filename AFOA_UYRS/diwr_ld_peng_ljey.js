...
async function diwr_ld_peng_ljey(diwr_slm, diwr_neig_kp) {
        ...
        var vwdp_vnwm = vnwm_ljey.map(async rn1 => {
            ...
            if (vxn_pzva === '电子控制单元') {
                ...
                return new Promise((resolve, reject) => {
                    ...
                    Promise.all([diwr_ld_peng_ljey(diwr_slm[rn1], diwr_neig_kp)]).then(jtyj => {
                        resolve(`"${rn1}":{\n${jtyj[0]}\n}`)
                    }).catch(err => {
                        reject(err)
                    })
...
module.exports = diwr_ld_peng_ljey