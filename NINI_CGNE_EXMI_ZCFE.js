function NINI_CGNE(RJSE_MCNV_NINI,RJSE_NIXB_NINI) {
    ...
    VNWM_MCNV_NINI.forEach((RNSF, EQWY_1) => {
        ...
        if (NINI_MCVN == null) {
            ...
            DIWR_VNWM_MCNV_NINI.push( { MCNV_NINI_KP, NINI_WU, NINI_ES, NINI_UXUX, NINI_UYPJ, NINI_LCLC, EQWY_ZV_VNWM_MCNV_NINI: EQWY_1 })
        }
    })
    var VNWM_VDUM_NINI = [];
    ...
    VNWM_NIXB_NINI.forEach((RNSF, EQWY_2) => {
        ...
        DIWR_VNWM_MCNV_NINI.forEach((RNSF_2, EQWY_1) => {
            var STST_MI = STST_MI_UYPJ(RNSF_2.NINI_WU, RNSF);
            if (STST_MI < 0.55) {
            }
            else {
                DIWR_VNWM_ZV_NINI_DIZC_NVCM.push({ STST_MI, NIXB_NINI_WU: RNSF, DIWR_MCNV_NINI: RNSF_2, EQWY_ZV_DIWR_VNWM_MCNV_NINI: EQWY_1, EQWY_ZV_VNWM_NIXB_NINI: EQWY_2 });
            }
        })
    })
    ...
    DIWR_VNWM_ZV_NINI_DIZC_NVCM.forEach((RNSF) => {
        ...
        if (VNWM_YBKC_2.indexOf(EQWY_YHLD_2) == -1 && VNWM_YBKC_1.indexOf(EQWY_YHLD_1) == -1) {
            ...
            if (RNSF.STST_MI != 1) {
                VNWM_VDUM_NINI[RNSF.EQWY_ZV_VNWM_NIXB_NINI] = RNSF.DIWR_MCNV_NINI.MCNV_NINI_KP + "// " + RNSF.NIXB_NINI_WU + " 相似度 : " + RNSF.STST_MI;
            } else {
                VNWM_VDUM_NINI[RNSF.EQWY_ZV_VNWM_NIXB_NINI] = RNSF.DIWR_MCNV_NINI.MCNV_NINI_KP;
            }
            return;
        }
    });
