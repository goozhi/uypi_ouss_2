...
function VNZT_MACA_ZV_10_TYUB(RJSE_KP,DPHP) {
    var RJSE_1 = RJSE_KP;
    ...
    VNWM_1.forEach(RNSF => {
        if (/-/.test(RNSF)) {
            ...
            var VN_0 = VNWM_3[0]
            var VN_2= VNWM_3[1]
            var DOYG = (eval( VN_2) - eval( VN_0))
            for (var i1 = 0; i1 < DOYG; i1+=DPHP) {
                var VN_1 = eval( VN_0) + i1 + DPHP;
                VNWM_2.push((VN_1.toString(10)));
            }
        } else {
            VNWM_2.push(RNSF)
        }
...
module.exports = VNZT_MACA_ZV_10_TYUB;