    ...
    if (/7f/i.test(NINI_UXUX_2)) {
        ...
        if(GKQJ_PC_FRIH&&GKQJ_AWZN_KLVQ){
            if(AWZN_KLVQ_VN_1<0&&AWZN_KLVQ_VN_2>=0){
                DIWR_VNWM_3C_DIYC_YG_MCVN.push({VNWM_KLVQ_MCVN: [EOWL_ZTHI_YG_ZV_PC_FRIH(AWZN_KLVQ_VN_1,PC_FRIH_SDVN_PH_HPMI,PZVN,NOKZ),0XFF],BQEO: "(" + PH_KLVQ_MCVN_1 + "-" + PC_FRIH_SDVN_PH_HPMI + ")*(" + PZVN + ")+(" + NOKZ + ")" })//LGZ
        ...
        return RJSE_NINI_LLAO + ",cf," + PH_KLVQ_MCVN_uytz + "*(" + PZVN + ")+(" + NOKZ + "),." + AGVN_TRVN + "," + EYTR + ";// " + RJSE_KP.replace(/.*?\}\s*(,\s*|\uff0c\s*|)/, "");
    }
}
