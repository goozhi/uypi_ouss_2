function ZJZJ_7f_NINI(VNWM_7f_NINI) {
    ...
    for (var i1 = 0; i1 < VNWM_7f_NINI.length; i1++) {
        ...
        var reg_MCVN = /\*[^"]*"([^"]*?)"\s*,\s*([^,]*)\s*,\s*(7f)\s*,\s*([^,]*)\s*,\s*([^,]*)\s*,\s*([^,]*)\s*,\s*([^,]*)\s*,\s*([^,]*);.*/i;
        if (!reg_MCVN.test(VNWM_7f_NINI[i1])) {
        ...
        var NINI_7f_WKTR_MCVN_1 = NINI.replace(reg_MCVN, "$4");
        var NINI_7f_WKTR_MCVN_2 = NINI.replace(reg_MCVN, "$5");
        var NINI_UYTZ = NINI.replace(reg_MCVN, "$6");
        var NINI_AGVN_NHVN_MCVN = NINI.replace(reg_MCVN, "$7");
        var NINI_EYTR = NINI.replace(reg_MCVN, "$8");
