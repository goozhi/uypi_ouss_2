...
function ZJZJ_TSJQ_Button(RJSE_KP) {
    var RJSE_MSOX = "";
    var ZJZJ_ES = require("./ZJZJ_ES");
    var RJSE_1 = RJSE_KP;
    var reg_Button_LD_MCVN = /\$Button\((.*)\)(?:;|)/i;
    var IOWR_Button_LD_MCVN = RJSE_1.match(reg_Button_LD_MCVN);
    if (IOWR_Button_LD_MCVN == null) {
        RJSE_MSOX += "\n<Button LGPH MSOX>\n" + RJSE_KP + "\n</Button LGPH MSOX>"
    } else {
        RJSE_1 = RJSE_1.replace(reg_Button_LD_MCVN, "");
    };
    if (!/^\s*$/.test(IOWR_Button_LD_MCVN[1])) {
        var reg_MCVN = /^\s*(?:[a-f]|\d)+\+-\s*$/i;
        if (!reg_MCVN.test(IOWR_Button_LD_MCVN[1])) {
            RJSE_MSOX += "[Button MCVN FTPJ] " + IOWR_Button_LD_MCVN[1] + "<--";
        } else {
