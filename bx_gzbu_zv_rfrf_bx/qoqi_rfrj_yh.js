...
const fs = require('fs')
async function qoqi_rfrj_yh(diwr_neig_kp) {
    var vnwm_fo_1 = [
        'nikc_bx',
        'vnwm_afoa_bqeo',
        'diwr_vnwm_bx',
    ...
    var vwdp_vnwm_1 = diwr_neig_kp.diwr_msqu.vnwm_dbkz_vdzv.map(async rn1 => {
        ...
        if (!diwr_qoqi) {
            ...
            if (!diwr_eynh) {
                ...
                var rj_yhld = diwr_eynh.yhrj
                if (!rj_yhld) {
                    uz_ms('csrf-nrap mcvn-' + rj_yhld)
                }
                try {
                    await zjzj_yhrd_diyc_gnpz({ yhrj: diwr_eynh.yhrj, rdrj: rdrj_yhld, DIWR_VNWM_reg_VWUX_MR_YFUX: diwr_vnwm_reg_vwux_mr_yfux })
                } catch (err) {
                ...
                diwr_eynh.rdrj = bqeo.replace(/^\s+|\s+$/g, "").replace(/Peng_JTCO_ZV_TZRN/g, "$$$$$$")
